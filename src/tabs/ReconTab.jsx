...
function ReconTab({ data, persist }) {
  ...
  if (empClose == null) {
    chainVerdict = { st:"none", color:"#888", text:"직원 마감액 미입력 — 퇴근 시 입력 유도" };
  } else if (kdAct == null) {
    chainVerdict = { st:"none", color:"#888", text:"카세북 마감(실제) 없음 — 직원 입력만 있음" };
  } else if (Math.abs(empVsAct) <= 1) {
    chainVerdict = { st:"ok", color:"#2f9e44", text:"직원 마감액 = 카세북 실제 (일치)" };
  } else {
    chainVerdict = { st:"bad", color:"#e03131", text:`직원 마감액과 카세북 실제 불일치 (${empVsAct>=0?"+":"−"}€${fmtE(Math.abs(empVsAct))}) — 오카운트/누락 확인` };
  }
  ...
  if (!hasSignal) {
    day = { label:"데이터 대기", color:"#888", bd:"#ccc", bg:"#fafafa" };
  } else if ((unexplained != null && gap.nurak > 5) || (kdDiff != null && kdDiff < -2)) {
    day = { label:"확인 필요", color:"#e03131", bd:"#ff6b6b", bg:"rgba(255,107,107,.05)" };
  } else if ((unexplained != null && gap.nurak > 1) || (kdDiff != null && kdDiff < -1)) {
    day = { label:"소액 차이", color:"#e8590c", bd:"#e8590c", bg:"rgba(232,89,12,.05)" };
  } else {
    day = { label:"정상", color:"#2f9e44", bd:"#2f9e44", bg:"rgba(47,158,68,.05)" };
  }
    ...
    <div>
      ...
      <div className="card" style={{border:`2px solid ${day.bd}`, background:day.bg, marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:8}}>
          ...
          <div style={{fontSize:15,fontWeight:800,color:day.color}}>{day.label}</div>
        </div>
            ...
            <Row label="미설명" main={`€${fmtE(unexplained)}`}
              sub={unexplained > 0 ? (
                <>
                  ...
                  {noReport ? <div style={{color:"#e8590c",fontWeight:700,marginTop:2}}>직원 리포트 0건 — 퇴근 때 설명을 안 남겼을 수 있어요</div> : null}
      ...
      <details ref={detailsRef} style={{marginBottom:12}}>
        ...
        <div style={{marginTop:10}}>
          ...
          <div className="card" style={{background:"rgba(77,171,247,.06)",border:"1px solid rgba(77,171,247,.35)",marginBottom:10,fontSize:11,color:"#555"}}>
            <strong>사진 구멍</strong> = 쿠폰(찍힌 사진) − SumUp사진 − 상품권. 무료·할인·재촬영·나약스로 설명하고 남는 <strong>미설명</strong>이 확인 대상.
                    ...
                    {un.map((s, i) => {
                      ...
                      return (
                        <tr key={i} style={marked ? {opacity:.55} : null}>
                          <td className="mn" style={{whiteSpace:"nowrap"}}>{s.t}</td>
                ...
                {checkShifts.map((s, i) => {
                  ...
                  return (
                    <div key={i} style={{fontSize:11,color:"#555",padding:"2px 0"}}>
                      ...
                      {s.checkConfirmed ? <span style={{color:"#2f9e44"}}> · 직원확인</span> : (u>=1 ? <span style={{color:"#e8590c"}}> · 미확인</span> : null)}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="card" style={{marginBottom:10}}>
            ...
            {!hasChain ? (
              ...
              <>
                ...
                <div className="card" style={{border:`1px solid ${chainVerdict.color}`, background: chainVerdict.st==="bad"?"rgba(255,107,107,.06)":(chainVerdict.st==="ok"?"rgba(47,158,68,.06)":"#f5f5f7"), display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div style={{fontSize:12,fontWeight:700,color:chainVerdict.color}}>{chainVerdict.text}</div>
                </div>
                {empOpen != null ? (
                  <div style={{fontSize:10,color:"#888",marginTop:8}}>오프닝이 인계한 €{fmtE(empOpen)}는 클로징이 이어받은 시재입니다.</div>
                ) : null}
              </>
            )}
          </div>
