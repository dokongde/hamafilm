...
6. 편집기에서 함수 `setupPushTrigger` 선택 → **실행** (권한 승인 창 나오면 승인)
   → 15분마다 `sendShiftReminders`가 돌기 시작
...
`push-notifications.gs` 맨 위 `REMINDER_MIN_BEFORE = 60` (기본: 시프트 시작 60분 전).
숫자만 바꾸고 저장하면 됨. 트리거 주기(15분)보다 큰 값 권장.

## 주의사항

- 서비스 계정 JSON은 **Apps Script 스크립트 속성에만** 보관 (레포/메신저 공유 금지)
- 직원이 홈화면 앱을 삭제하면 토큰이 만료됨 — 발송 실패 시 자동으로 구독 목록에서 제거되니 방치해도 됨
- 무료 한도: FCM 무제한, Apps Script 트리거 하루 90분 실행(15분 주기 실행으로 충분히 여유)
